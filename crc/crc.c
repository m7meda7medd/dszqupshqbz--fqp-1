...
uint32_t CRC32_Init(CRC_Handle_t* hcrc) {
    if (hcrc->crc_poly == 0){ /* Check if the crc not initialized */
...
uint32_t CRC32_Calculate(CRC_Handle_t* hcrc, uint8_t *data, uint32_t length) {
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        for (int j = 0; j < 8; j++) {
            if (crc & 1) {  // If the least significant bit is 1
                crc = (crc >> 1) ^ hcrc->crc_poly;  // Shift right and XOR with the polynomial
            } else {
